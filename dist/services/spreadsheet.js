 ...
 */
function buildCartaoPontoWorkbook(transcription) {
 ...
 */
function buildHoleriteWorkbook(transcription) {
    const workbook = new ExcelJS.Workbook();
 ...
 */
export async function generateSpreadsheet(tipo, value, formato) {
    if (formato === 'json') {
        return {
    ...
    const workbook = tipo === 'cartao-ponto'
        ? buildCartaoPontoWorkbook(value)
        : buildHoleriteWorkbook(value);
