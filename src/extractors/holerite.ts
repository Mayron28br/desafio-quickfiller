import { HoleriteBase, HoleriteField, HoleritePage, HoleriteTranscription } from '../types/index.js';
 ...
 */
function formatBaseLabel(text: string): string {
  const clean = text.replace(/[\:\-\|]/g, ' ').replace(/\s+/g, ' ').trim();
  const words = clean.split(' ');
  return words.map(w => {
    const upper = w.toUpperCase();
    if (upper === 'INSS' || upper === 'FGTS' || upper === 'IRRF' || upper === 'IR') return upper;
    if (upper === 'DE' || upper === 'DO' || upper === 'DA' || upper === 'DOS' || upper === 'DAS' || upper === 'A') return w.toLowerCase();
 ...
 */
function getBaseMatch(text: string, hasCode: boolean): { isBase: boolean; label: string } {
 ...
 */
export function extractCompetence(text: string): { month: string; year: string } {
 ...
 */
export function parseHolerite(document: ExtractedDocument): HoleriteTranscription {
      ...
      for (const seg of segments) {
        ...
        if (finalLabel.length >= 2 && (hasCode || !getBaseMatch(finalLabel, false).isBase)) {
          const alreadyExists = fields.some(f =>
            (f.label.toLowerCase() === finalLabel.toLowerCase() || (code && f.code === code)) &&
            f.value === seg.value &&
