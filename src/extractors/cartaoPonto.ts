import { CartaoPontoPage, CartaoPontoTranscription, DayRecord, Punch, PunchKind, RegistroCartaoPonto } from '../types/index.js';
import { ExtractedDocument, ExtractedPage, OcrWord } from '../services/ocr.js';
...
export interface HeaderContext {
  month?: string;
  year?: string;
  periodStart?: { day: number; month: number; year: number };
  periodEnd?: { day: number; month: number; year: number };
}
...
/**
  ...
  */
export function extractHeaderContext(text: string): HeaderContext {
 ...
 */
export function formatToFullDate(raw: string | number, context?: HeaderContext): string {
 ...
 */
export function sanitizeTimeToken(raw: string): string[] {
  if (!raw) return [];
  let s = raw.trim();
...
export function normalizeTime(raw: string): string {
  ...
  if (trimmed.includes('?')) {
    ...
    if (qMatches.length > 0) return qMatches[0]!;
...
export function buildDayRecord(date_raw: string, punches: Punch[]): DayRecord {
  return {
    date_raw,
    punches,
 ...
 */
function parseBancoDoBrasilPage(text: string, pageNumber: number, context?: HeaderContext): CartaoPontoPage | null {
 ...
 */
function parseSiponPage(text: string, pageNumber: number, context?: HeaderContext): CartaoPontoPage | null {
  const pageCtx = { ...context, ...extractHeaderContext(text) };
  const lines = text.split('\n').map(l => l.trim()).filter(Boolean);
  const days: DayRecord[] = [];
 ...
 */
export function parseQuinzenaWithBBox(page: ExtractedPage, context?: HeaderContext): CartaoPontoPage[] | null {
  ...
  const processQuinzenaWords = (qWords: OcrWord[], startDay: number, endDay: number, pageNum: number): CartaoPontoPage => {
 ...
 */
function parseQuinzenaPage(text: string, pageNumber: number, context?: HeaderContext): CartaoPontoPage | null {
  const pageCtx = { ...context, ...extractHeaderContext(text) };
  const lines = text.split('\n').map(l => l.trim()).filter(Boolean);
 ...
 */
function parseStandardColumnarPage(text: string, pageNumber: number, context?: HeaderContext): CartaoPontoPage {
  const pageCtx = { ...context, ...extractHeaderContext(text) };
  const lines = text.split('\n').map(l => l.trim()).filter(Boolean);
  const days: DayRecord[] = [];
